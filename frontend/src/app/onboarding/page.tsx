...
export default function Onboarding() {
  ...
  const rmZone = (i: number) => setZones(zones.filter((_, idx) => idx !== i));
  const updZone = (i: number, k: string, v: string) =>
    setZones(zones.map((z, idx) => idx === i ? { ...z, [k]: v } : z));
  ...
  const finish = async () => {
    ...
    try {
      ...
      toast.error(e?.response?.data?.detail || "Failed to create farm");
    } finally {
      setBusy(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-emerald-50 to-stone-100 py-8 px-4">
      <div className="max-w-2xl mx-auto">
        ...
        <Card className="rounded-2xl shadow-md border border-stone-200 bg-white">
          <CardContent className="p-6 sm:p-8">
            {step === 1 && (
              <div>
                ...
                <div className="flex justify-end">
                  <Button
                    data-testid="onboard-step-1-next"
                    onClick={() => setStep(2)}
            ...
            {step === 2 && (
              <div>
                ...
                <p className="text-sm text-stone-600 mb-4">Fill what you know. You can add the rest later.</p>
                <div className="grid grid-cols-2 gap-3">
                  <div className="col-span-2">
                    <Label>{t(lang, "farm_name")}</Label>
                    <Input
                      data-testid="onboard-farm-name"
                      value={farm.name}
                  ...
                  <div className="col-span-2">
                    ...
                    <Input
                      data-testid="onboard-farm-location"
                      value={farm.location}
                  ...
                  <div className="col-span-2">
                    <Label>{t(lang, "water")}</Label>
                    <Input
                      data-testid="onboard-water"
                      value={farm.water_availability}
                      onChange={(e) => setFarm({ ...farm, water_availability: e.target.value })}
                      placeholder="Borewell, Tank, Canal..."
                    />
                  </div>
                  <div className="col-span-2">
                    <Label>{t(lang, "irrigation")}</Label>
                    <Input
                      data-testid="onboard-irr"
                      value={farm.irrigation_method}
                      onChange={(e) => setFarm({ ...farm, irrigation_method: e.target.value })}
                      placeholder="Drip, Sprinkler, Flood..."
                    />
                  </div>
                  <div className="col-span-2">
                    <Label>{t(lang, "farming_type")}</Label>
                    <Input
                      data-testid="onboard-type"
                      value={farm.farming_type}
                      onChange={(e) => setFarm({ ...farm, farming_type: e.target.value })}
                      placeholder="Organic, Conventional, Mixed..."
                    />
                  </div>
                </div>
                <div className="flex justify-between mt-6">
                  ...
                  <Button
                    data-testid="onboard-step-2-next"
                    onClick={() => setStep(3)}
                    className="bg-emerald-700 hover:bg-emerald-800"
                    disabled={!farm.name}
                  >
                  ...
                  {zones.map((z, i) => (
                    <div key={i} className="p-3 rounded-xl border border-stone-200 bg-stone-50">
                      <div className="grid grid-cols-2 gap-2">
                        <Input
                          data-testid={`zone-name-${i}`}
                          placeholder={t(lang, "zone_name")}
                          value={z.name}
                          onChange={(e) => updZone(i, "name", e.target.value)}
                        />
                        <Input
                          data-testid={`zone-area-${i}`}
                          placeholder={t(lang, "area")}
                          type="number"
                          step="0.1"
                          value={z.area}
                          onChange={(e) => updZone(i, "area", e.target.value)}
                        />
                        <Input
                          data-testid={`zone-crop-${i}`}
                          placeholder={t(lang, "zone_crop")}
                          value={z.crop}
                          onChange={(e) => updZone(i, "crop", e.target.value)}
                        />
                        <Input
                          data-testid={`zone-soil-${i}`}
                          placeholder={t(lang, "zone_soil")}
                          value={z.soil_type}
                          onChange={(e) => updZone(i, "soil_type", e.target.value)}
                        />
                      </div>
                      {zones.length > 1 && (
                        <button
                          ...
                          onClick={() => rmZone(i)}
                          className="mt-2 text-xs text-rose-600 flex items-center gap-1 cursor-pointer"
                        >
