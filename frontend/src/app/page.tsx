...
import { Leaf, Camera, Mic, Layers, Cloud, ArrowRight, Sparkles } from "lucide-react";
import {
    ...
    <div className="min-h-screen bg-gradient-to-br from-stone-50 via-emerald-50 to-teal-50">
      <header className="flex items-center justify-between px-6 py-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="font-extrabold tracking-tight text-emerald-900 text-xl leading-none">AGRiNEX</div>
            <div className="text-[10px] text-stone-500 uppercase tracking-widest mt-0.5">Agriculture + Next Gen</div>
          </div>
        </div>
        <div className="flex items-center gap-2">
          ...
          <Button data-testid="landing-login-btn" variant="outline" onClick={() => nav("/auth")}>
            {t(lang, "login")}
          </Button>
        </div>
      </header>
          ...
          <div className="relative rounded-3xl overflow-hidden shadow-2xl border border-white">
            ...
            <div className="absolute bottom-4 left-4 right-4 bg-white/95 backdrop-blur rounded-2xl p-4 shadow-lg">
              ...
              <div className="grid grid-cols-4 gap-1.5">
                {[
                  { bg: "bg-emerald-100", border: "border-emerald-300", dot: "bg-emerald-500" },
                  { bg: "bg-amber-100", border: "border-amber-300", dot: "bg-amber-500" },
                  { bg: "bg-emerald-100", border: "border-emerald-300", dot: "bg-emerald-500" },
                  { bg: "bg-blue-100", border: "border-blue-300", dot: "bg-blue-500" },
                ].map((c, i) => (
                  <div key={i} className={`h-10 rounded ${c.bg} border ${c.border} flex items-center justify-center`}>
