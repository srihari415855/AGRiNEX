...
export function ZoneCard({ zone, onClick }) {
  const s = STATUS_STYLES[zone.status] || STATUS_STYLES.no_data;
  return (
    <button
      onClick={onClick}
      data-testid={`zone-card-${zone.id}`}
      className={`group relative text-left w-full rounded-2xl border-2 ring-2 ring-offset-2 ring-offset-stone-50 ${s.ring} bg-gradient-to-br ${s.bg} border-white/60 p-4 shadow-sm hover:shadow-lg transition-all hover:-translate-y-0.5`}
    >
...
export default function DigitalTwin({ zones, onZoneClick, isDemo }) {
  ...
  return (
    <div className="relative rounded-3xl bg-gradient-to-br from-emerald-950 via-emerald-900 to-teal-900 p-6 sm:p-8 shadow-2xl overflow-hidden">
      ...
      <div className="relative">
        {(!zones || zones.length === 0) ? (
          <div className="bg-white/10 backdrop-blur-sm rounded-xl p-8 text-center text-emerald-100">
            ...
            {!isDemo && (
              <button data-testid="twin-go-setup" onClick={() => nav("/onboarding")} className="px-4 py-2 rounded-lg bg-white text-emerald-800 font-semibold text-sm">
                Setup Farm
              </button>
            )}
          </div>
      ...
      <div className="relative mt-6 flex items-center gap-3 flex-wrap text-xs text-emerald-100">
        ...
        <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-blue-400" />Irrigating</span>
        <span className="flex items-center gap-1.5"><span className="w-2 h-2 rounded-full bg-stone-400" />No data</span>
      </div>
    </div>
  );
}
